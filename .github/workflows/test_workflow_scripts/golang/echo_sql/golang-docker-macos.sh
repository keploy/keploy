...
for i in {1..2}; do
    ...
    send_request &
    $RECORD_BIN record -c "docker compose up" --container-name "$container_name" --generateGithubActions=false --proxy-port=$PROXY_PORT --dns-port=$DNS_PORT 2>&1 | tee "${container_name}.txt"
...
$REPLAY_BIN test -c 'docker compose up' --containerName "$test_container" --apiTimeout 60 --delay 10 --generate-github-actions=false --proxy-port=$PROXY_PORT --dns-port=$DNS_PORT 2>&1 | tee "${test_container}.txt"

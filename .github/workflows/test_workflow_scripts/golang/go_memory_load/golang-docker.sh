...
run_with_keploy_privileges "$RECORD_BIN" record -c "docker compose up" --container-name "$APP_CONTAINER_NAME" --memory-limit 200 --enable-sampling --generate-github-actions=false 2>&1 | tee record.txt &
record_pid=$!

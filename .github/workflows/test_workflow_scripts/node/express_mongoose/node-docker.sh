...
sudo -E env PATH=$PATH $REPLAY_BIN test -c "docker run --rm --name $test_container --network keploy-network node-app:1.0" --containerName "$test_container" --apiTimeout 30 --delay 40 --fallBackOnMiss=false --disableTele --generate-github-actions=false &> "${test_container}.txt"

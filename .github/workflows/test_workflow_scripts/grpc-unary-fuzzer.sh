...
if [ "$MODE" == "incoming" ]; then
    ...
    sudo -E env PATH="$PATH" $RECORD_BIN record -c "go run cmd/server/main.go" &> record_incoming.txt &
    sleep 10
    
    # Start client
    go run ./cmd/client --http :18080 &> client_incoming.txt &
    sleep 10
    ...
    sudo -E env PATH="$PATH" $REPLAY_BIN test -c "go run cmd/server/main.go" &> test_incoming.txt
    ...
    # Start server
    go run cmd/server/main.go &> server_outgoing.txt &
    sleep 5

    # Start client in record mode
    sudo -E env PATH="$PATH" $RECORD_BIN record -c "go run ./cmd/client --http :18080" &> record_outgoing.txt &
    sleep 10
    ...
    sudo -E env PATH="$PATH" $REPLAY_BIN test -c "go run ./cmd/client --http :18080" &> test_outgoing.txt

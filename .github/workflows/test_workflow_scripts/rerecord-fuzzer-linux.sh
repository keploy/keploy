...
sudo $KEPLOY_BIN config --generate

# --- 1. Record Phase ---
section "Start Recording Server"
# Start keploy record in the background and redirect output to a log file
sudo -E env PATH="$PATH" $KEPLOY_BIN record -c $RERECORD_SERVER_BIN > record.log 2>&1 &
KEPLOY_PID=$!
...
# Run rerecord non-interactively by piping 'yes' to answer prompts automatically
printf 'y\ny\n' | sudo -E env PATH="$PATH" ASSERT_CHAINS_WITH="$ASSERT_CHAINS_WITH" \
$KEPLOY_BIN rerecord -c $RERECORD_SERVER_BIN -t "test-set-0" --show-diff \
  > rerecord.log 2>&1
RERECORD_RC=$?
cat rerecord.log

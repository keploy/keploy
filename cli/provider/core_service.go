...
import (
	...
	"go.keploy.io/server/v3/pkg/service/diagnose"
...
func Get(ctx context.Context, cmd string, cfg *config.Config, logger *zap.Logger, tel *telemetry.Telemetry, auth service.Auth) (interface{}, error) {
	...
	diagnoseSvc := diagnose.New(logger, cfg, commonServices.YamlReportDb, commonServices.YamlTestDB)
	switch cmd {
	case "rerecord":

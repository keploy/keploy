...
func (c *CmdConfigurator) AddUncommonFlags(cmd *cobra.Command) {
	...
	case "test":
		...
		cmd.Flags().Bool("schema-noise-detection", c.cfg.Test.SchemaNoiseDetection, "Detect request-body fields that drift between recording and replay and persist them as field-path noise (req_body_noise) during auto-replay matching. Available to any parser that implements the shared schema-noise adapter (HTTP, plus Pulsar in the enterprise build)")
		cmd.Flags().Bool("schema-noise-strict", c.cfg.Test.SchemaNoiseStrict, "Strictly enforce learned request-body noise during mock matching: a candidate mock carrying req_body_noise is rejected when any field OUTSIDE its learned/user-configured noise drifted. Available to any parser that implements the shared schema-noise adapter (HTTP, plus Pulsar in the enterprise build). Same behaviour the in-cluster replay path enforces; previously configurable only via keploy.yml")

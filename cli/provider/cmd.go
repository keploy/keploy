...
func (c *CmdConfigurator) AddFlags(cmd *cobra.Command) error {
	...
	case "record", "test", "rerecord":
		...
		cmd.Flags().DurationP("build-delay", "b", c.cfg.BuildDelay, "User provided time to wait docker container build (e.g., \"5s\" for 5 seconds, \"1m\" for 1 minute)")
...
func (c *CmdConfigurator) AddUncommonFlags(cmd *cobra.Command) {
	...
	case "test", "rerecord":
		...
		if cmd.Name() == "test" {
			cmd.Flags().DurationP("delay", "d", 5*time.Second, "User provided time to run its application")

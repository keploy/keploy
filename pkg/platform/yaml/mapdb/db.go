...
func (db *MappingDb) Upsert(ctx context.Context, testSetID string, testID string, mockEntrys []models.MockEntry) error {
	...
	for i, t := range mapping.Tests {
		if t.ID == testID {
			...
			mapping.Tests[i].Mocks = mockEntrys
			found = true
			break
		}
	}
	...
	db.logger.Debug("Successfully upserted test-mock mapping",
		...
		zap.Int("mockCount", len(mockEntrys)))

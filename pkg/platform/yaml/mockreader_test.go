...
// TestMockFileReader_LargeFile tests reading a large mock file (3GB)
func TestMockFileReader_LargeFile(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping large file test in short mode")
	}

	// Check available disk space and memory
	t.Log("=== Large Mock File Reader Test (3GB) ===")

	// Create temp directory
	tempDir, err := os.MkdirTemp("", "keploy-mock-test-*")
	if err != nil {
		t.Fatalf("Failed to create temp directory: %v", err)
	}
	defer func() {
		t.Log("Cleaning up temp directory...")
	...
	// Generate 3GB mock file
	t.Log("Generating 3GB mock file...")
	_, expectedMocks := generateLargeMockFile(t, tempDir, 3.0)

	// Force GC before reading to get accurate memory baseline
	runtime.GC()
	var memStatsBefore runtime.MemStats
	runtime.ReadMemStats(&memStatsBefore)
	t.Logf("Memory before reading: Alloc=%.2f MB, Sys=%.2f MB",
		float64(memStatsBefore.Alloc)/(1024*1024),
		float64(memStatsBefore.Sys)/(1024*1024))

	// Test 1: MockFileReader (Scanner-based)
	t.Log("\n--- Test 1: MockFileReader (Scanner-based) ---")
	testMockFileReader(t, ctx, logger, tempDir, expectedMocks)

	// Test 2: LineBasedMockReader
	t.Log("\n--- Test 2: LineBasedMockReader ---")
	testLineBasedMockReader(t, ctx, logger, tempDir, expectedMocks)
}

func testMockFileReader(t *testing.T, ctx context.Context, logger *zap.Logger, tempDir string, expectedMocks int) {
	runtime.GC()
	...
	reader, err := NewMockFileReader(ctx, logger, tempDir, "mocks")
	if err != nil {
		t.Fatalf("Failed to create MockFileReader: %v", err)
	}
	defer reader.Close()
	...
	for {
		...
		if doc.Kind == "" {
			t.Errorf("Mock %d has empty kind", mockCount)
		}
	...
	t.Logf("MockFileReader completed in %v", elapsed)
...
// TestMockFileReader_SmallFile is a quick sanity test with a small file
func TestMockFileReader_SmallFile(t *testing.T) {
	tempDir, err := os.MkdirTemp("", "keploy-mock-test-small-*")
	if err != nil {
		t.Fatalf("Failed to create temp directory: %v", err)
	}
	...
	_, expectedMocks := generateLargeMockFile(t, tempDir, 0.01)
	...
	for {
		...
		if doc.Kind != "Mongo" {
			t.Errorf("Expected kind 'Mongo', got '%s'", doc.Kind)
		}
		count++
	}

	if count != expectedMocks {
		t.Errorf("Mock count mismatch: got %d, want %d", count, expectedMocks)
	}
	t.Logf("Successfully read %d mocks from small test file", count)
}

// TestMockFileReader_ContextCancellation tests that context cancellation works
func TestMockFileReader_ContextCancellation(t *testing.T) {
	tempDir, err := os.MkdirTemp("", "keploy-mock-test-cancel-*")
	if err != nil {
		t.Fatalf("Failed to create temp directory: %v", err)
	}
	defer os.RemoveAll(tempDir)
	...
	// Generate small file
	generateLargeMockFile(t, tempDir, 0.001)
	...
	// Next read should return context error
	_, err = reader.ReadNextDoc()
...
// BenchmarkMockFileReader benchmarks the reader performance
func BenchmarkMockFileReader(b *testing.B) {
	tempDir, err := os.MkdirTemp("", "keploy-mock-bench-*")
	if err != nil {
	...
	// Generate 100MB file for benchmarking
	filePath := filepath.Join(tempDir, "mocks.yaml")
	file, _ := os.Create(filePath)
	file.WriteString("# Generated by Keploy (bench)\n")
	for i := 0; i < 5000; i++ {
		if i > 0 {
			file.WriteString("---\n")
		}
		file.WriteString(fmt.Sprintf(mockTemplate, i, i%100, i*2+1, i*2+1))
	}
	file.Close()
	...
	for i := 0; i < b.N; i++ {
		reader, _ := NewMockFileReader(ctx, logger, tempDir, "mocks")
		for {
			_, err := reader.ReadNextDoc()
			if err == io.EOF {
				break

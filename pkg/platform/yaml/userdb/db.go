// Package userdb provides functionality for working with keploy configuration databases.
package userdb
...
func NewUserDb(logger *zap.Logger) *UserDb {
	return &UserDb{
		logger: logger,
	}
}

func (cdb *UserDb) GetInstallationID(ctx context.Context) (string, error) {
	var id string
...
func (cdb *UserDb) setInstallationID(ctx context.Context, id string) error {
	path := UserHomeDir()
	data := []byte{}

...
func (ys *MockYaml) UpdateTestMockReferences(ctx context.Context, testSetID string, usedMocksByTests map[string][]string) error {
	mockFileName := "mocks"
	if ys.MockName != "" {
		mockFileName = ys.MockName
	}
	...
	var mocksTestCases = make(map[string][]string)
	for testName, mockState := range usedMocksByTests {
		for _, state := range mockState {
			if _, ok := mocksTestCases[state]; !ok {
				mocksTestCases[state] = []string{testName}
			} else {
				mocksTestCases[state] = append(mocksTestCases[state], testName)
			}
		}
	}

	for _, mock := range mocks {
		if testCases, ok := mocksTestCases[mock.Name]; ok {
			mock.UsedByTests = testCases
		}
	}

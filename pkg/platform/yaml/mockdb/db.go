...
func (ys *MockYaml) filterByTimeStamp(_ context.Context, m []*models.Mock, afterTime time.Time, beforeTime time.Time, logger *zap.Logger) ([]*models.Mock, []*models.Mock) {
	...
	if time.Time.Equal(afterTime, time.Time{}) {
		return m, unfilteredMocks
	}

	if time.Time.Equal(beforeTime, time.Time{}) {
		return m, unfilteredMocks
	}
	...
	for _, mock := range m {
		...
		if time.Time.Equal(mock.Spec.ReqTimestampMock, time.Time{}) || time.Time.Equal(mock.Spec.ResTimestampMock, time.Time{}) {

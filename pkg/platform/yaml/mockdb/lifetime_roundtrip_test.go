...
var lifetimeRoundTripCases = []lifetimeCase{
	...
	{
		// The header NAME probe is case-insensitive. Proxy captures are
		// canonicalised by pkg.ToYamlHTTPHeader, but mocks arriving as raw
		// JSON/YAML — cloud sync, hand-written fixtures — carry whatever
		// casing the producer emitted, and this is the layer they arrive on.
		name:     "http_client_preflight_lowercase_header_name_is_session",
		kind:     models.HTTP,
		metadata: map[string]string{"type": "HTTP_CLIENT", "operation": "OPTIONS"},
		method:   "OPTIONS",
		url:      "http://svc/query",
	...
	{
		// Methods are case-sensitive (RFC 9110 section 9.1) and
		// http.ReadRequest preserves the wire casing verbatim, so a lowercase
		// "options" reaches disk unchanged and must not be promoted.
		name:     "http_client_lowercase_options_method_is_per_test",
		kind:     models.HTTP,
		metadata: map[string]string{"type": "HTTP_CLIENT", "operation": "OPTIONS"},
		method:   "options",
		url:      "http://svc/query",
	...
	{
		// A preflight always names the method it is asking about, so an empty
		// Access-Control-Request-Method is malformed rather than a preflight.
		// Pins that the empty VALUE survives the round trip and is rejected on
		// its own merits, not because the key went missing.
		name:     "http_client_preflight_empty_acrm_value_is_per_test",
		kind:     models.HTTP,
		metadata: map[string]string{"type": "HTTP_CLIENT", "operation": "OPTIONS"},
		method:   "OPTIONS",
		url:      "http://svc/query",
	...
	{
		// REGRESSION GUARD for the gob read path. This row must PASS in both
		// formats; a failure here is a real regression, not an expected result.
		//
		...
		// embed it), so the reloaded mock has LifetimeDerived=false and
		// DeriveLifetime runs normally.
		//
		// gob does NOT: it ignores struct tags and encodes every exported
		// field, so LifetimeDerived=true would reach mocks.gob, and
		// DeriveLifetime would return at its first statement
		// (pkg/models/lifetime.go:131-133) without running ANY rule — leaving
		// the stale per-test classification, so the preflight lands in the
		// per-test pool, is consumed on first match, and is invisible to every
		// other test. That is regression B28, and it would silently disable
		// rule 5 as well, not only rule 1(b).
		//
		// readGobMocks clears both fields after decode (db.go, see the comment
		// on the reset) precisely so this cannot happen. THIS ROW IS THE ONLY
		// THING GUARDING THAT RESET: if someone removes it as redundant, this
		// row is what should turn red.
...
//
// NOTE: gob is the format that needs this most. It ignores struct tags, so
// without readGobMocks clearing the runtime-only TestModeInfo fields after
// decode, the recorder's LifetimeDerived stamp would survive and short-circuit
// DeriveLifetime entirely. The http_recorder_prestamped_preflight_is_session
// row is the guard for that; it must pass here.
...
func runLifetimeRoundTrip(t *testing.T, gobFormat bool) {
	...
	// Guard against a vacuous pass: assert the corpus is non-empty and
	// survived the round trip before asserting anything about lifetimes.
	if len(included) == 0 {
		t.Fatalf("every row was excluded (strict=%v, gobFormat=%v) — this test would assert nothing", strict, gobFormat)
	}
	...
	for _, c := range included {
		...
		t.Run(c.name, func(t *testing.T) {
			...
			if c.preStamped {
				msg += "\n\n" + strings.Join([]string{
					"REGRESSION: the runtime-only TestModeInfo fields are surviving a reload.",
					"This row pre-stamps TestModeInfo{Lifetime, LifetimeDerived: true} exactly as the",
					"recorders do at emit time (pkg/agent/proxy/integrations/http/http.go:331-334,",
					"recordv2.go:576-579). gob ignores the `json:\"-\" bson:\"-\"` tags and encodes every",
					"exported field, so LifetimeDerived=true reaches mocks.gob; readGobMocks must clear",
					"it after decode, otherwise DeriveLifetime returns at pkg/models/lifetime.go:131-133",
					"and NO classification rule runs at all — disabling both the CORS-preflight promotion",
					"(rule 1(b)) and the lax kind fallback (rule 5) for every gob recording.",
					"Most likely cause: the reset in readGobMocks was removed or bypassed, or a new gob",
					"decode site for models.Mock was added without it. yaml/json cannot regress this way",
					"because NetworkTrafficDoc does not carry TestModeInfo at all.",
				}, "\n")
			}
			t.Fatalf("%s", msg)
		})

...
var _ = Describe("Factory", func() {
	...
	Describe("When tracker is malformed", func() {
		It("should be able to delete the tracker", func() {
			...
			factory := NewFactory(inactivityThreshold, respChannel, logger)
			// TODO: Replace with counterfeiter.
			db := yaml.NewYamlStore("fakeTCpath", "fakeMockPath", logger)

			getDeps := func() []*models.Mock {
				return nil
			}

			dependenciesResetCount := 0
			resetDeps := func() int {
				dependenciesResetCount += 1
				return -1
			}
			...
			factory.HandleReadyConnections(db, getDeps, resetDeps)

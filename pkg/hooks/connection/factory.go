...
// HandleReadyConnections investigates the list of present trackers and performs certain operations on them.
// After this operation, all trackers would
func (factory *Factory) HandleReadyConnections(db platform.TestCaseDB, getDeps func() []*models.Mock, resetDeps func() int) {
	...
	for connID, tracker := range factory.connections {
		if tracker.IsComplete() {
			...
			parsedHttpReq, err := utils.ParseHTTPRequest(tracker.recvBuf)
			if err != nil {
				factory.logger.Error("failed to parse the http request from byte array", zap.Error(err))
				continue
			}

			parsedHttpRes, err := utils.ParseHTTPResponse(tracker.sentBuf, parsedHttpReq)
			if err != nil {
			...
			default:
				factory.logger.Warn("Keploy mode is not set to record/test. Tracker is being skipped.",

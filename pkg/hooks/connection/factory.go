...
func (factory *Factory) HandleReadyConnections(db platform.TestCaseDB, ctx context.Context, url []string, header []string) {
	factory.mutex.Lock()
	...
	for connID, tracker := range factory.connections {
		...
		if ok {
			...
			case models.MODE_RECORD:
				...
				capture(db, parsedHttpReq, parsedHttpRes, factory.logger, ctx, reqTimestampTest, resTimestampTest, url, header)
...
func capture(db platform.TestCaseDB, req *http.Request, resp *http.Response, logger *zap.Logger, ctx context.Context, reqTimeTest time.Time, resTimeTest time.Time, url []string, header []string) {

...
func (h *HTTP) recordV2(ctx context.Context, sess *supervisor.Session) error {
	...
	for {
		...
		// 101 Switching Protocols: everything after this on the connection is
		// the upgraded protocol (WebSocket frames, h2c), not HTTP. Looping back
		// to read "the next request" would block until the hang watchdog retires
		// the parser 60s later. Stop cleanly instead, having recorded the
		// upgrade exchange itself.
		if httpStatusCode(finalResp) == 101 {
			logger.Debug("V2 HTTP record: connection upgraded (101); stopping HTTP parsing for it")
			return nil
		}
	}
}
...
func (h *HTTP) readRequestV2(ctx context.Context, stream *fakeconn.FakeConn, finalReq *[]byte) error {
	...
	if transferEncodingHeader != "" &&
		...
		for !chunkedBodyComplete(*finalReq) {
			if err := ctx.Err(); err != nil {
				return err
			}
			chunk, err := stream.ReadChunk()
...
func (h *HTTP) readResponseV2(ctx context.Context, stream *fakeconn.FakeConn, finalResp *[]byte, reqMethod string) (time.Time, error) {
	...
	// an upgrade, and the caller stops parsing the connection after it.
	...
	if transferEncodingHeader != "" &&
		...
		for !chunkedBodyComplete(*finalResp) {
			if err := ctx.Err(); err != nil {
				return lastWr, err
			}
			chunk, err := stream.ReadChunk()
...
// chunkedBodyComplete reports whether a chunked body is terminated. It accepts
// both the bare last chunk "0\r\n\r\n" and a last chunk followed by trailer
// fields (RFC 9112 section 7.1.2), e.g. "0\r\nX-Sum: 9\r\n\r\n", which a plain
// HasSuffix on the terminator never matches — leaving the parser blocked for the
// life of the connection.
func chunkedBodyComplete(buf []byte) bool {
	if bytes.HasSuffix(buf, chunkedTerminator) {
		return true
	}
	// A trailer section: the last-chunk marker somewhere earlier, and the
	// message ending on the blank line that closes the trailer fields.

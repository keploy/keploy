...
//
// NAMING: this package was called schemanoise until the feature was renamed to
// "mock noise" across flags, config keys, identifiers and logs. Because it
// crosses a MODULE boundary — github.com/keploy/integrations and
// github.com/keploy/enterprise both import it — the old import path still
// exists next door as a deprecated forwarding shim built on type aliases, so
// callers on either path share one set of types and can migrate independently.
// The shim goes away once both consumers are on this path.
//
...
//
//	type myAdapter struct{ mocknoise.JSONDiffer }
//

...
// schema-noise. learned is in the shared "body."-prefixed vocabulary and is
// stored on the kind-agnostic MockSpec.ReqBodyNoise (same store HTTP uses).
// Equal payload length to the live request drives the matcher into the
// structure-equal branch.
func dmlMock(name, sql string, learned map[string][]string) *models.Mock {
	m := readbackMock(name, sql, "served", time.Time{})
	m.TestModeInfo.Lifetime = models.LifetimePerTest
	m.Spec.Metadata = map[string]string{"type": "mocks"}
	m.Spec.ReqBodyNoise = learned
	return m
}

// TestMatchCommand_ComQueryLiteralNoise exercises the end-to-end COM_QUERY
// request-literal noise wiring through matchCommand on the shared schema-noise
// engine: detection learns and attaches noise (MockSpec.ReqBodyNoise) onto the
// consumed mock; strict enforcement consumes a structurally-identical mock iff
// every literal drift is in the learned-noise set; and a WHERE-only drift never
// matches under strict.
func TestMatchCommand_ComQueryLiteralNoise(t *testing.T) {
	...
	t.Run("detection attaches learned noise onto consumed mock", func(t *testing.T) {
		...
		// that DeleteFilteredMock consumed — on MockSpec.ReqBodyNoise.
		...
		got := db.deletedMk[0].Spec.ReqBodyNoise
		...
		if mock.Spec.ReqBodyNoise != nil {
			t.Errorf("detection mutated the shared pooled mock's ReqBodyNoise: %v", mock.Spec.ReqBodyNoise)
		}
	})

	t.Run("strict consumes mock when only learned-noise literal drifts", func(t *testing.T) {
		learned := map[string][]string{"body.set:updated_at#0": {"2026-01-01 12:48:36"}}
...
// when the live query's structural counterpart is found but drifted in a
// non-noise / predicate literal (a real regression), strict mode must force an
// OVERALL MISS — it must NOT fall back to serving an unrelated candidate that
// merely scored a payload-length partial.
...
// under strict, a candidate whose redacted skeleton differs from the live query
// (changed column / operator, or a different statement shape) must NOT be served
// through matchCommand's score-based partial fallback even when payload lengths
// coincide. Strict serves only an exact or within-noise match; otherwise it must
// MISS so the real query-shape regression surfaces.

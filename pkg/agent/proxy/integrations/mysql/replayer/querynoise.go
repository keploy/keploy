...
//
// The returned keys are the raw clause-role keys ("set:<col>#<n>" /
// "values:<row>:<col>"). The schema-noise adapter (schema_noise.go) is
// responsible for adding the shared "body." prefix and subtracting
// already-learned keys before the engine records them on MockSpec.ReqBodyNoise.
...
//
// learned is keyed by the raw clause-role key ("set:<col>#<n>" /
// "values:<row>:<col>") — the caller strips the shared "body." prefix off the
// mock's stored MockSpec.ReqBodyNoise (via Engine.KnownNoise) before passing it.

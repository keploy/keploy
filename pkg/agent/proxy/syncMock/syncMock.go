...
func (m *SyncMockManager) AddMock(mock *models.Mock) {
	...
	if !m.firstReqSeen && m.outChan != nil {
		...
		// silently lost. The runtime message for this specific panic
		// is stable across Go versions: it is generated by the
		// `chansend1` fast path with a constant "send on closed
		// channel" plainError.
		func() {
			defer func() {
				r := recover()
				...
				if err, ok := r.(runtime.Error); ok && err.Error() == "send on closed channel" {
					return
				}
				panic(r)
			}()

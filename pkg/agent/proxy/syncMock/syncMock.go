...
func (m *SyncMockManager) AddMock(mock *models.Mock) {
	...
	m.mu.Lock()
	if m.memoryPause {
		// Snapshot fields for logging while we still hold the lock; the log
		// itself fires after Unlock to keep the lock window tight.
		pressureStart := m.currentPressureStart
		m.mu.Unlock()
		if logger := m.dropLogger(); logger != nil && mock != nil {
			logger.Info("diag/AddMock: mock dropped — memory pressure active when AddMock was called",
				zap.String("mock_kind", string(mock.Kind)),
				zap.String("mock_name", mock.Name),
				zap.Time("req_time", mock.Spec.ReqTimestampMock),
				zap.Time("res_time", mock.Spec.ResTimestampMock),
				zap.Time("pressure_start", pressureStart),
			)
		}
		return
	}

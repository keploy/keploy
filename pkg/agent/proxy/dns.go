...
func (p *Proxy) startTCPDNSServer(_ context.Context) error {
	...
	if err != nil {
		proxyErr := NewDNSProxyError("tcp", p.DNSPort, err)
		utils.LogError(p.logger, proxyErr, "failed to start tcp dns server", zap.String("addr", server.Addr))
		return proxyErr
	}
	return nil
}
...
func (p *Proxy) startUDPDNSServer(_ context.Context) error {
	...
	err := server.ListenAndServe()
	if err != nil {
		proxyErr := NewDNSProxyError("udp", p.DNSPort, err)
		utils.LogError(p.logger, proxyErr, "failed to start udp dns server", zap.String("addr", server.Addr))
		return proxyErr
	}
	return nil
}
...
func (p *Proxy) resolveUncachedDNSResponse(question dns.Question, mode models.Mode, mockingEnabled bool, reqTime time.Time, session *agent.Session) dnsCacheEntry {
	...
	case models.MODE_TEST:
		...
		if fwdResp, fwdErr := p.forwardDNSUpstream(question); fwdErr == nil && fwdResp != nil {
			...
			// the app: any unknown name should resolve to the proxy IP
			// so eBPF can intercept it and match against recorded mocks

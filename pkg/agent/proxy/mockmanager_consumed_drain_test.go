...
// TestGetPersistentConsumed_DoesNotDrain pins the complement of the contract
...
//
// So: GetPersistentConsumed accumulates across the manager's lifetime and never
// empties, and reading it leaves the drain untouched for the summary.
func TestGetPersistentConsumed_DoesNotDrain(t *testing.T) {
	mm := NewMockManager(nil, nil, zap.NewNop())
	defer mm.Close()

	if got := mm.GetPersistentConsumed(); len(got) != 0 {
	...
	for i := 0; i < 3; i++ {
		total := mm.GetPersistentConsumed()
		if len(total) != 2 {
	...
	delete(total, "http-1")
	if len(mm.GetPersistentConsumed()) != 2 {
		t.Fatal("GetPersistentConsumed must return a copy, not the live map")
	}
}

...
func HandleTLSConnection(ctx context.Context, logger *zap.Logger, conn net.Conn, backdate time.Time) (net.Conn, bool, error) {
	...
	// Determine whether to negotiate ALPN based on the proxy mode.
	// In record mode we use dynamic ALPN so that gRPC-over-TLS clients
	// (grpc-go >= 1.67 requires h2) can connect through the proxy.
	// In test mode we skip ALPN to preserve the exact TLS behaviour that
	// was present when the mocks were recorded — otherwise calls that
	// originally failed at ALPN would now succeed and hit the mock
	// server with no matching mock.
	proxyMode, _ := ctx.Value(models.ProxyModeKey).(models.Mode)

	var config *tls.Config
	if proxyMode == models.MODE_TEST {
		config = &tls.Config{

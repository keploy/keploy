...
// Used by the proxy to write the CA cert to a temp file for the Rust proxy.
func GetCACertPEM() []byte {
	return caCrt
}

// ── PEM-based cert cache for Rust IPC ───────────────────────────────────
// This mirrors the tls.Certificate cache but stores PEM bytes so they can
// be sent to the Rust proxy over IPC.
...
// signed by the embedded CA, returning the PEM-encoded cert and key.
// This is used by the IPC server to respond to the Rust proxy's GET_CERT requests.
func CertForHost(logger *zap.Logger, serverName string, backdate time.Time) (certPEM, keyPEM []byte, err error) {
	if serverName == "" {
		serverName = "localhost"
	}

	// Fast path: PEM cache hit
	if c, k, ok := getCachedPEM(serverName); ok {
		logger.Debug("PEM cert cache hit", zap.String("host", serverName))
		return c, k, nil
	}

	// Ensure CFSSL log level is set
	setLogLevelOnce.Do(func() {
		cfsslLog.Level = cfsslLog.LevelError
	})
	...
	if err != nil {
		return nil, nil, fmt.Errorf("failed to get parsed CA: %w", err)
	}
	...
	serverCsr, serverKey, err := csr.ParseRequest(serverReq)
	if err != nil {
		return nil, nil, fmt.Errorf("failed to create server CSR: %w", err)
	}

	cryptoSigner, ok := caPrivKey.(crypto.Signer)
	if !ok {
		return nil, nil, fmt.Errorf("failed to typecast the caPrivKey")
	}
	...
	// Cache PEM for future requests
	setCachedPEM(serverName, serverCert, serverKey)
	logger.Debug("PEM cert generated and cached", zap.String("host", serverName))

...
type Service interface {
	Get(ctx context.Context, summary bool, cid , user string, app, id *string, from, to *time.Time,offset *int, limit *int) ([]*TestRun, error)
	Put(ctx context.Context, run TestRun) error
	Normalize(ctx context.Context, cid, id string) error
}

type DB interface {
	Read(ctx context.Context, cid , user string, app, id *string, from, to *time.Time,offset *int, limit *int) ([]*TestRun, error)

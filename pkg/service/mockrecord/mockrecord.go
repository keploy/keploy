...
func (s *mockRecorder) MockRecord(path string, proxyPort uint32, pid uint32, mockName string, disablTele bool) {

	models.SetMode(models.MODE_RECORD)
	teleFS := fs.NewTeleFS(s.logger)
	tele := telemetry.NewTelemetry(!disablTele, false, teleFS, s.logger, "", nil)
	tele.Ping(false)

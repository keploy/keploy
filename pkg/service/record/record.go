...
func (r *Recorder) Start(ctx context.Context, reRecordCfg models.ReRecordCfg) error {
	...
	err := r.instrumentation.Setup(setupCtx, r.config.Command, models.SetupOptions{Container: r.config.ContainerName, DockerDelay: r.config.BuildDelay, Mode: models.MODE_RECORD, CommandType: r.config.CommandType, EnableTesting: false, GlobalPassthrough: r.config.Record.GlobalPassthrough, BuildDelay: r.config.BuildDelay, PassThroughPorts: passPortsUint, EnableDockerUnconfined: r.config.Agent.EnableDockerUnconfined, ConfigPath: r.config.ConfigPath, EnableSampling: r.config.Record.EnableSampling})

...
const (
	HighReasoning   ReasoningType = "high"   // HighReasoning represents high reasoning effort.
	LowReasoning    ReasoningType = "low"    // LowReasoning represents low reasoning effort.
	MediumReasoning ReasoningType = "medium" // MediumReasoning represents medium reasoning effort.
)
...
func (ai *AIClient) Call(ctx context.Context, completionParams CompletionParams, aiRequest AIRequest, stream bool) (string, error) {
	...
	ai.Logger.Debug("Streaming results from LLM model...")
	if stream {
		for {

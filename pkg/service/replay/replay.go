	...
	for replay := 0; replay < itr; replay++ {
		...
		for idx, testCase := range testsToRun {
			...
			if isCurrentTestStreaming {
				tcCopy := *testCase
				expectedMocksForTCEntries := expectedTestMockMappings[testCase.Name]
				expectedMocksForTC := make([]string, len(expectedMocksForTCEntries))
				for i, m := range expectedMocksForTCEntries {
					expectedMocksForTC[i] = m.Name
				}

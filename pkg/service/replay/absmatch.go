...
func AbsMatch(tcs1, tcs2 *models.TestCase, noiseConfig map[string]map[string][]string, ignoreOrdering bool, logger *zap.Logger) (bool, bool, bool, *models.AbsResult) {
	...
	//compare http req
	reqPass, reqResult := CompareHTTPReq(tcs1, tcs2, noiseConfig, ignoreOrdering, logger)
	if !reqPass {
	...
	//compare http resp
	respPass, respResult := CompareHTTPResp(tcs1, tcs2, noiseConfig, ignoreOrdering, logger)
	if !respPass {
...
func CompareHTTPReq(tcs1, tcs2 *models.TestCase, _ models.GlobalNoise, ignoreOrdering bool, logger *zap.Logger) (bool, models.ReqResult) {
	pass := true
	//compare http req
	...
	if tcs1.HTTPReq.Method != tcs2.HTTPReq.Method {
		reqResult.MethodResult.Normal = false
	...
	if tcs1.HTTPReq.URL != tcs2.HTTPReq.URL {
		reqResult.URLResult.Normal = false
	...
	if tcs1.HTTPReq.ProtoMajor != tcs2.HTTPReq.ProtoMajor {
		reqResult.ProtoMajor.Normal = false
	...
	if tcs1.HTTPReq.ProtoMinor != tcs2.HTTPReq.ProtoMinor {
		reqResult.ProtoMinor.Normal = false
	...
	if bodyType1 != bodyType2 {
		...
		pass = false
		reqResult.BodyResult.Normal = false
...
func CompareHTTPResp(tcs1, tcs2 *models.TestCase, noiseConfig models.GlobalNoise, ignoreOrdering bool, logger *zap.Logger) (bool, models.RespResult) {
	pass := true
	//compare http resp
	...
	if tcs1.HTTPResp.StatusCode != tcs2.HTTPResp.StatusCode {
		respResult.StatusCode.Normal = false
	...
	if !ok {
		...
		pass = false
		respResult.BodyResult.Normal = false

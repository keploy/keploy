...
func (s *server) Serve(path, testReportPath string, Delay uint64, pid, port uint32) {
	...
	// start the proxy
	ps := proxy.BootProxy(s.logger, proxy.Option{}, "", "")
	...
	go func() {
		...
		s.logger.Debug(Emoji + "graphql server stopped")
	}()
	...
	s.logger.Info(Emoji + "Received signal, initiating graceful shutdown...")

	// Gracefully shut down the HTTP server with a timeout
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	if err := httpSrv.Shutdown(ctx); err != nil {
		s.logger.Error(Emoji+"Graphql server shutdown failed", zap.Error(err))
	}

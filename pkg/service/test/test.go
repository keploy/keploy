...
func (t *tester) Test(path, testReportPath string, appCmd, appContainer, appNetwork string, Delay uint64) bool {
	...
	t.logger.Debug(Emoji + fmt.Sprintf("the session indices are:%v", sessions))
...
func (t *tester) RunTestSet(testSet, path, testReportPath, appCmd, appContainer, appNetwork string, delay uint64, pid uint32, ys platform.TestCaseDB, loadedHooks *hooks.Hook, testReportFS yaml.TestReportFS, testRunChan chan string) bool {
	...
	t.logger.Debug(Emoji + fmt.Sprintf("the testcases for %s are: %v", testSet, tcs))
	...
	t.logger.Debug(Emoji + fmt.Sprintf("the config mocks for %s are: %v\nthe testcase mocks are: %v", testSet, configMocks, tcsMocks))
	loadedHooks.SetConfigMocks(configMocks)
	loadedHooks.SetTcsMocks(tcsMocks)

	t.logger.Debug(Emoji, zap.Any("app pid", pid))
	if len(appCmd) == 0 && pid != 0 {
		t.logger.Debug(Emoji + "running keploy tests along with other unit tests")
	} else {
	...
	if len(appCmd) == 0 && pid != 0 {
		t.logger.Debug(Emoji + "no need to stop the user application when running keploy tests along with unit tests")
	} else {

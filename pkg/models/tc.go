...
type TestCasesWithCount struct {
	TestCases []TestCase
	Count     int64
}
...
type TestCaseDB interface {
	...
	GetAll(ctx context.Context, cid, app string, anchors bool, offset int, limit int) (*TestCasesWithCount, error)
	...
	//CountDocuments(ctx context.Context, cid string) (int64, error)
}

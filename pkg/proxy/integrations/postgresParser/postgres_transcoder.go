...
type BackendWrapper struct {
	BackendWrapper models.Backend // Embed MyType from packageA
}

type FrontendWrapper struct {
	FrontendWrapper models.Frontend // Embed MyType from packageA
...
func (f *FrontendWrapper) TranslateToReadableResponse(msgBody []byte, logger *zap.Logger) (pgproto3.BackendMessage, error) {
	...
	// f.FrontendWrapper.AuthMechanism = fmt.Sprint(f.FrontendWrapper.AuthType)
	// bits := msg.Encode([]byte{})

	// if len(bits) != len(msgBody) {
	// 	logger.Error("Encoded Data doesn't match the original data ..")
	// }

...
func ProcessOutgoingMongo(connId int, requestBuffer []byte, clientConn, destConn net.Conn, h *hooks.Hook, started time.Time, readRequestDelay time.Duration, logger *zap.Logger) {
	switch models.GetMode() {
	case models.MODE_RECORD:
		// capturedDeps := encodeOutgoingMongo(requestBuffer, clientConn, destConn, logger)
		encodeOutgoingMongo(connId, requestBuffer, clientConn, destConn, h, started, readRequestDelay, logger)
...
func encodeOutgoingMongo(destConnId int, requestBuffer []byte, clientConn, destConn net.Conn, h *hooks.Hook, started time.Time, readRequestDelay time.Duration, logger *zap.Logger) {
	rand.Seed(time.Now().UnixNano())
	connId := rand.Intn(101)
	for {

		var err error
		var logStr string = fmt.Sprintln("the connection id: ", connId, " the destination conn id: ", destConnId)
		...
		if string(requestBuffer) == "read form client connection" {
			...
			if err != nil {
				logger.Error(Emoji+"failed to read request from the mongo client", zap.Error(err), zap.String("mongo client address", clientConn.RemoteAddr().String()), zap.Any("client conn id", connId), zap.Any("dest conn id", destConnId))
				return
			}
		...
		opReq, requestHeader, mongoRequest, err := Decode((requestBuffer))
		if err != nil {
			logger.Error(Emoji+"failed to decode the mongo wire message from the client", zap.Error(err), zap.Any("client conn id", connId), zap.Any("dest conn id", destConnId))
			return
		}
		...
		if val, ok := mongoRequest.(*models.MongoOpMessage); ok && hasSecondSetBit(val.FlagBits) {
			for {
				...
				logStr += tmpStr
				if err != nil {
					logger.Error(Emoji+"failed to read reply from the mongo server", zap.Error(err), zap.String("mongo server address", destConn.RemoteAddr().String()), zap.Any("client conn id", connId), zap.Any("dest conn id", destConnId))
					return
				}
				readRequestDelay = time.Since(started)
				...
				if err != nil {
					fmt.Println(logStr)
					logger.Error(Emoji+"failed to write the reply message to mongo client", zap.Error(err), zap.Any("client conn id", connId), zap.Any("dest conn id", destConnId))
					return
				}
				...
				_, reqHeader, mongoReq, err := Decode(requestBuffer1)
				if err != nil {
					logger.Error(Emoji+"failed to decode the mongo wire message from the destination server", zap.Error(err), zap.Any("client conn id", connId), zap.Any("dest conn id", destConnId))
					return
				}
		...
		if err != nil {
			logger.Error(Emoji+"failed to write the request buffer to mongo server", zap.Error(err), zap.String("mongo server address", destConn.RemoteAddr().String()), zap.Any("client conn id", connId), zap.Any("dest conn id", destConnId))
			return
		}
		...
		// fmt.Println("before decoding the heartbeat request")
		// _, _, _, err = Decode(requestBuffer)
		// fmt.Println("before decoding the heartbeat response")
		// _, _, _, err = Decode(responseBuffer)
		...
		// 	return nil
		// }
		// fmt.Println("after hertbeat interchange. the reQuest")
		...
		// 	return nil
		// }
		// fmt.Println("after hertbeat interchange. the response")
		// Decode(msgResponseBuffer)
		...
		_, responseHeader, mongoResponse, err := Decode(responseBuffer)
		if err != nil {
			logger.Error(Emoji+"failed to decode the mongo wire message from the destination server", zap.Error(err), zap.Any("client conn id", connId), zap.Any("dest conn id", destConnId))
			return
		}
		...
		if val, ok := mongoResponse.(*models.MongoOpMessage); ok && hasSecondSetBit(val.FlagBits) {
			// fmt.Println("into the more_to_come", logStr)
			// if isHeartBeat(opReq) {
			// 	select {
			// 	case <-time.After(1 * time.Minute):
			// 		fmt.Println("appending the heartbeat calls")
			// 		go recordMessage(h, requestBuffer, responseBuffer, logStr, mongoRequests, mongoResponses, opReq)
			// 	}
			// }
			for {
				if isHeartBeat(opReq) && time.Since(started) >= 5*time.Minute {
					go recordMessage(h, requestBuffer, responseBuffer, logStr, mongoRequests, mongoResponses, opReq)
				}
				fmt.Println("into the more_to_come", logStr)
				tmpStr := ""
				...
				_, respHeader, mongoResp, err := Decode(responseBuffer1)
				if err != nil {
					logger.Error(Emoji+"failed to decode the mongo wire message from the destination server", zap.Error(err), zap.Any("client conn id", connId), zap.Any("dest conn id", destConnId))
					return
				}
		...
		go recordMessage(h, requestBuffer, responseBuffer, logStr, mongoRequests, mongoResponses, opReq)
		// go func(requestBuffer, responseBuffer []byte, logStr string, clientConnId, destConnId int) {
		// 	fmt.Println(logStr)
		...
		// 				if mongoRequest.(*models.MongoOpQuery).Query == v {
		// 					shouldRecordCalls = false
		// 					break
		// 				}
		// 				configRequests = append(configRequests, mongoRequest.(*models.MongoOpQuery).Query)
		// 			case wiremessage.OpMsg:
		// 				if len(mongoRequest.(*models.MongoOpMessage).Sections) > 0 && mongoRequest.(*models.MongoOpMessage).Sections[0] == v {
		// 					shouldRecordCalls = false
		// 					break
		// 				}
		// 				configRequests = append(configRequests, mongoRequest.(*models.MongoOpMessage).Sections[0])
		// 			default:
		// 				if opReq.String() == v {
		// 					shouldRecordCalls = false
		// 					break
		...
		// 			}
		// 			// if opReq.String() == v {
		// 			// if (requestHeader.Opcode == wiremessage.OpQuery && mongoRequest.(*models.MongoOpQuery).Query == v) || (requestHeader.Opcode == wiremessage.OpMsg && len(mongoRequest.(*models.MongoOpMessage).Sections)>0 && mongoRequest.(*models.MongoOpMessage).Sections[0] == v){
		// 			// shouldRecordCalls = false
		// 			// isHeartbeatRecorded = true
		// 			// 	break
		// 			// }
		// 			// configRequests = append(configRequests, opReq.String())
		// 		}
		// 		// if !isHeartbeatRecorded {

		// 		// }
		// 		// shouldRecordCalls
		// 	}
		// 	// } else {
		// 	// _, responseHeader, mongoResp, err := Decode(responseBuffer)
		// 	// if err != nil {
		...
		// 			},
		// 		}
		// 		h.AppendDeps(mongoMock)
		// 	}
		// }(requestBuffer, responseBuffer, logStr, connId, destConnId)
		requestBuffer = []byte("read form client connection")
	...
	// if err != nil {
	// 	logger.Error(Emoji+"failed to read request from the mongo client", zap.Error(err), zap.String("mongo client address", clientConn.RemoteAddr().String()))
	// 	return
	// }
	...
	// 	// util.ReadBytes(destConn)
	// 	return
	// }
	// ------------------------------------------------
	// encodeOutgoingMongo([]byte("read form client connection"), clientConn, destConn, h, logger)

	// deps := []*models.Mock{}

	// decode the mongo binary request wiremessage
	// logger.Info(Emoji , zap.String("the heartbeat request buffer: ", opr.String()))

	// decode the mongo binary response wiremessage
	// 	// logger.Info(Emoji , zap.String("the heartbeat response buffer: ", op.String()))
...
// Skip heartbeat from capturing in the global set of mocks. Since, the heartbeat packet always contain the "hello" boolean.
// See: https://github.com/mongodb/mongo-go-driver/blob/8489898c64a2d8c2e2160006eb851a11a9db9e9d/x/mongo/driver/operation/hello.go#L503
func isHeartBeat(opReq Operation) bool {
	return strings.Contains(opReq.String(), "helloOk") || strings.Contains(opReq.String(), "hello")
}

...
// ParseBodyToStreamEntries detects the streaming type from headers and parses
// the raw response body into structured StreamBodyEntry entries. If the response
// is not streaming, returns nil.
func ParseBodyToStreamEntries(body string, headers map[string]string, timestamp time.Time) []models.StreamBodyEntry {
	contentType := getHeaderValueCaseInsensitive(headers, "Content-Type")
	contentTypeLower := strings.ToLower(contentType)

	ts := timestamp.Format(time.RFC3339Nano)

	if strings.Contains(contentTypeLower, "text/event-stream") {
		return parseSSEBodyToEntries(body, ts)
	}
	if strings.Contains(contentTypeLower, "application/x-ndjson") || strings.Contains(contentTypeLower, "application/ndjson") {
		return parseNDJSONBodyToEntries(body, ts)
	}
	if strings.Contains(contentTypeLower, "multipart/x-mixed-replace") || strings.Contains(contentTypeLower, "multipart/mixed") {
		return parseHTTPRawChunksToEntries(body, ts)
	}

	// Check for chunked transfer encoding with text/plain or octet-stream
	transferEncoding := strings.ToLower(getHeaderValueCaseInsensitive(headers, "Transfer-Encoding"))
	if strings.Contains(transferEncoding, "chunked") {
		if strings.Contains(contentTypeLower, "text/plain") ||
			strings.Contains(contentTypeLower, "application/octet-stream") {
			return parseHTTPRawChunksToEntries(body, ts)
		}
	}
...
// parseSSEBodyToEntries parses a raw SSE body (frames separated by double newlines)
// into structured StreamBodyEntry entries.
func parseSSEBodyToEntries(body string, ts string) []models.StreamBodyEntry {
	...
	entries := make([]models.StreamBodyEntry, 0, len(frames))
	for _, frame := range frames {
		entry := models.StreamBodyEntry{
			Ts:   ts,
		...
		// Check if it's a comment-only frame
		allComments := true
		...
		if allComments && len(fields) == 1 {
			entry.Data["comment"] = strings.TrimSpace(fields[0].value)
			entries = append(entries, entry)
			continue
		}

		// Multi-field frame: merge comment lines and data lines
		var dataLines []string
		var commentParts []string
		for _, f := range fields {
			if f.comment {
				commentParts = append(commentParts, strings.TrimSpace(f.value))
				continue
			}
			switch f.key {
			case "id":
				entry.Data["id"] = f.value
			case "event":
				entry.Data["event"] = f.value
			case "retry":
				entry.Data["retry"] = f.value
			case "data":
				dataLines = append(dataLines, f.value)
			}
		}
		if len(commentParts) > 0 {
			entry.Data["comment"] = strings.Join(commentParts, "\n")
		}
		if len(dataLines) > 0 {
			entry.Data["data"] = strings.Join(dataLines, "\n")
		}
...
// parseNDJSONBodyToEntries parses an NDJSON body into StreamBodyEntry entries
// with each JSON line stored under "raw".
func parseNDJSONBodyToEntries(body string, ts string) []models.StreamBodyEntry {
	body = normalizeLineEndings(body)
	...
	entries := make([]models.StreamBodyEntry, 0, len(lines))
	for _, line := range lines {
		entries = append(entries, models.StreamBodyEntry{
			Ts:   ts,
			Data: map[string]string{"raw": line},
		})
	}
	return entries
}

// parseHTTPRawChunksToEntries parses a plain text / binary / multipart body
// into StreamBodyEntry entries with each line/chunk stored under "raw".
func parseHTTPRawChunksToEntries(body string, ts string) []models.StreamBodyEntry {
	body = normalizeLineEndings(body)
...
func IsStreamingContentType(headers map[string]string) bool {
	contentType := strings.ToLower(getHeaderValueCaseInsensitive(headers, "Content-Type"))

...
func (g *graph) Serve(path string, proxyPort uint32, mongopassword, testReportPath string, Delay uint64, pid, port uint32, lang string, passThroughPorts []uint, apiTimeout uint64, appCmd string, enableTele bool, deleteTestReport bool) {
	...
	defer utils.DeleteTestReport(g.logger, deleteTestReport)

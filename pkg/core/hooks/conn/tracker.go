...
func (conn *Tracker) validateDataSizes(exp, act uint64) bool {
...
func (conn *Tracker) isHTTP1Complete() (bool, []byte, []byte, time.Time, time.Time) {
	...
	if conn.recTestCounter > 0 {
		if (len(conn.userReqSizes) > 0 && len(conn.kernelReqSizes) > 0) &&
			...
			if conn.validateDataSizes(expectedRecvBytes, actualRecvBytes) {
				validReq = true
			} else {
			...
			if conn.validateDataSizes(expectedSentBytes, actualSentBytes) {
				validRes = true

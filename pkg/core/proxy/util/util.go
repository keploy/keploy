...
// ReadBytes function is utilized to read the complete message from the reader until the end of the file (EOF).
// It returns the content as a byte array.
func ReadBytesMongo(ctx context.Context, logger *zap.Logger, reader io.Reader) ([]byte, error) {
...
// It returns the content as a byte array.
func ReadRequiredBytesMongo(ctx context.Context, logger *zap.Logger, reader io.Reader, numBytes int) ([]byte, error) {
	if numBytes <= 0 {
		return nil, nil
	}

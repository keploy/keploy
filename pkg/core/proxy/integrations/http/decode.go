...
func (h *HTTP) decodeHTTP(ctx context.Context, reqBuf []byte, clientConn net.Conn, dstCfg *models.ConditionalDstCfg, mockDb integrations.MockMemDb, opts models.OutgoingOptions, logger *zap.Logger) error {
	...
	go func(errCh chan error, reqBuf []byte, opts models.OutgoingOptions) {
		...
		for {
			...
			err := h.HandleChunkedRequests(ctx, &reqBuf, clientConn, nil, logger)
			if err != nil {
			...
			if err != nil {
				utils.LogError(logger, err, "failed to read from request body", zap.Any("metadata", utils.GetReqMeta(request)))
				errCh <- err
				return
			}
			...
			if err != nil {
				utils.LogError(logger, err, "error while matching http mocks", zap.Any("metadata", utils.GetReqMeta(request)))
				errCh <- err
				return
			}
			...
			if !ok {
				if !utils.IsPassThrough(logger, request, dstCfg.Port, opts) {
					utils.LogError(logger, nil, "Didn't match any preExisting http mock", zap.Any("metadata", utils.GetReqMeta(request)))
				}
				if opts.FallBackOnMiss {
					_, err = pUtil.PassThrough(ctx, logger, clientConn, dstCfg, [][]byte{reqBuf})
					if err != nil {
						utils.LogError(logger, err, "failed to passThrough http request", zap.Any("metadata", utils.GetReqMeta(request)))
						errCh <- err
						return
					}
				}
			...
			if stub == nil {
				utils.LogError(logger, nil, "matched mock is nil", zap.Any("metadata", utils.GetReqMeta(request)))
			...
			if header["Content-Encoding"] != nil && header["Content-Encoding"][0] == "gzip" {
				...
				if err != nil {
					utils.LogError(logger, err, "failed to compress the response body", zap.Any("metadata", utils.GetReqMeta(request)))
					errCh <- err
					return
				}
				err = gw.Close()
				if err != nil {
					utils.LogError(logger, err, "failed to close the gzip writer", zap.Any("metadata", utils.GetReqMeta(request)))
					errCh <- err
					return
				}
			...
			if err != nil {
				...
				utils.LogError(logger, err, "failed to write the mock output to the user application", zap.Any("metadata", utils.GetReqMeta(request)))
				errCh <- err
				return
			}

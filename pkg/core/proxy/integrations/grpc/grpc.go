...
func (g *Grpc) RecordOutgoing(ctx context.Context, src net.Conn, dst net.Conn, mocks chan<- *models.Mock, opts models.OutgoingOptions) error {
	logger := g.logger.With(zap.Reflect("Client ConnectionID", ctx.Value(models.ClientConnectionIDKey).(string)), zap.Reflect("Destination ConnectionID", ctx.Value(models.DestConnectionIDKey).(string)), zap.Reflect("Client IP Address", src.RemoteAddr().String()))

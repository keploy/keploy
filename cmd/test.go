...
func (t *Test) GetCmd() *cobra.Command {
	var testCmd = &cobra.Command{
		...
		Example: `sudo -E keploy test -c "/path/to/user/app" --startup-time 6`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if appCmd == "" {
				...
				if isDockerCmd {
					fmt.Println("Example usage:\n", `keploy test -c "docker run -p 8080:808 --network myNetworkName --rm myApplicationImageName" --startup-time 6\n`)
				}
				fmt.Println("Example usage:\n", cmd.Example, "\n")
			...
			if delay <= 5 {
				fmt.Printf("Warning: delay is set to %d seconds, incase your app takes more time to start udse --startup-time to set custom delay\n", delay)
				if isDockerCmd {
					fmt.Println("Example usage:\n", `keploy test -c "docker run -p 8080:808 --network myNetworkName --rm myApplicationImageName" --startup-time 6\n`)
				} else {
					fmt.Println("Example usage:\n", cmd.Example, "\n")

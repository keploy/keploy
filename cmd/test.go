...
func (t *Test) getTestConfig(path *string, proxyPort *uint32, appCmd *string, tests *map[string][]string, appContainer, networkName *string, Delay *uint64, buildDelay *time.Duration, passThroughPorts *[]uint, apiTimeout *uint64, globalNoise *models.GlobalNoise, testSetNoise *models.TestsetNoise, coverageReportPath *string, withCoverage *bool, configPath string, ignoreOrdering *bool, passThroughHosts *[]models.Filters, EnableANSIColor *bool) error {
...
func (t *Test) GetCmd() *cobra.Command {
	var testCmd = &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			EnableANSIColor, err := cmd.Flags().GetBool("enableANSIColor")
			if err != nil {
				t.logger.Error("Failed to get the enableANSIColor flag", zap.Error((err)))
				return err
			}
			...
			err = t.getTestConfig(&path, &proxyPort, &appCmd, &tests, &appContainer, &networkName, &delay, &buildDelay, &ports, &apiTimeout, &globalNoise, &testsetNoise, &coverageReportPath, &withCoverage, configPath, &ignoreOrdering, &passThroughHosts, &EnableANSIColor)
			if err != nil {

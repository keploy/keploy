...
// ExecCancelWithTimeout cancels the context and waits for the shutdown to complete within the specified timeout.
// It returns an error if the shutdown does not complete within the timeout or if the cancel function is not set.
func ExecCancelWithTimeout(timeout time.Duration) error {
	...
	cancel()
	// Create a timer to ensure shutdown completes within timeout
	timer := time.NewTimer(timeout)
	defer timer.Stop()
	select {
	case <-timer.C:
		return fmt.Errorf("context cancellation did not complete within %s", timeout)
	default:
		return nil
	}
}
